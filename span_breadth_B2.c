...
int spanning_tree_breadth_B2(V* graph,int nVertices,int max_d_node,THREADED)
{
...
#define MYCOLOR (MYTHREAD+1)
  ...
  int i,j,n,root,walks,r,counter=0,visited, neighbor,n_neighbors;
  double power;
  ...
  if(graph[max_d_node].n_neighbors>DEG) root=max_d_node;
  else  root=(rand_r(&seed)%(nVertices));
  node_Barrier();
  ...
  while(first_time || work_to_steal)
    {
      /*if(work_to_steal) printf("THREAD %d:stealing work\n",MYTHREAD);*/
	  ...
	  for(i=0;i<graph[n].n_neighbors;i++)
	    {
	      ...
	      if(!color[neighbor]) {/*found new frontier*/
		color[neighbor]=MYCOLOR;
      ...
      if(MYTHREAD%2==0)
	{
	  for(j=0;j<THREADS;j++)
		...
		r=b+(n-b)/THREADS;
	      ...
	      if(r<(*top_M[i])) {
		(*bottom_M[i])=min(max((r-1),-1),*top_M[i]);
  ...
  on_one_thread{
    ...
    printf("METRICS:===span_breadth_B2:The difference between counts is %d\n",max-min);
  }
  node_Barrier();
