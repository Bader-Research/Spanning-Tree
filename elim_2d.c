...
int eliminate_2d_vertices(V* G, int n_vertices,int * root,THREADED)
{
  ...
  double interval;
  int  *count_matrix;
  count_matrix=node_malloc(sizeof(int)*THREADS,TH);
  ...
  if(count_matrix[0]<n_vertices/THREADS){
    ...
    node_free(count_matrix,TH);
    return (0);
  }
  
  starting_point=node_malloc(sizeof(int),TH);
  ...
  node_Barrier();
  *(root)=(*starting_point);
  node_free(starting_point,TH);
  return 1;
}

int in_my_range(int i, int n, THREADED)
{
