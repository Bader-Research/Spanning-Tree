...
int spanning_tree_breadth_B1(V* graph, int nVertices,THREADED)
{
...
#define INDEX(a,b) ((a-1)*(THREADS)+(b-1))
  ...
  int * color, first_time, work_to_steal,myroot,b,max, host, *stack,top,count,bottom=-1,i,j,n,root,walks,r,*starting_points,counter=0;
  double power;
  ...
  finished = node_malloc(THREADS*sizeof(int),TH);
  finished[MYTHREAD]=0;
